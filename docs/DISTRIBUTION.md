...
jobs:
  gate:
    ...
    steps:
      ...
      - uses: jvega017/claude-provenance@main
```
...
`jvega017/claude-provenance@v0.10.0`) once you depend on it.
...
```yaml
repos:
  - repo: https://github.com/jvega017/claude-provenance
    rev: v0.10.0
    hooks:
...
published. Until then install from `main` or from PyPI (`pipx install
warrantos`).
