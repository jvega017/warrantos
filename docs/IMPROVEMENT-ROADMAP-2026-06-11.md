...
6. **Integrate the pre-publish gate into the brief pipeline.** Substantial, high impact. `tools/warrantos-pre-publish-gate.ps1` calls `warrantos check --profile brief-light --ci`, exits non-zero on HOLD/BLOCK/NOT_ASSESSABLE, appends the JSON verdict to `publish-gate-shadow.log`; the estate publication pipeline calls it before writing output. This converts the shadow observer from observation to blocking. Files: `tools/warrantos-pre-publish-gate.ps1` (new), the external estate publication script, and `docs/QUICKSTART.md`.
